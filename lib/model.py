...
def test_data_loader(FLAGS):
    ...
    def preprocess_test(name, mode):
        im = matplotlib.pyplot.imread(name, mode="RGB").astype(np.float32)
...
def VGG19_slim(input, type, reuse, scope):
    ...
    if type == 'VGG54':
        target_layer = 'vgg_19/conv5/conv5_4' # scope + 'vgg_19/conv5/conv5_4'
    elif type == 'VGG22':
        target_layer = 'vgg_19/conv2/conv2_2' # scope + 'vgg_19/conv2/conv2_2'
    else:
